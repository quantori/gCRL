...
class GCRLVAE(nn.Module):
    ...
    def _dag_transform(self, z, bc, csz, bc2=None, csz2=None):
        """
        Apply the causal DAG transform to latent variables using the full adjacency matrix G.

        We solve (I - G_offdiag) u = y, where:
            - G_offdiag is G with its diagonal zeroed (no self-loops)
            - y is the observational latent z plus intervention shifts

        This works for any (learned) acyclic G; acyclicity is enforced via a NOTEARS-style
        penalty during training.
        """
        B, K = z.shape
        # Use full adjacency matrix, removing self-loops
        G = self.G
