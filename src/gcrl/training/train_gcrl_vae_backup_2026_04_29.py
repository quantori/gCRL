...
LOSS FUNCTION (6 components):
    ...
    Key insight: Centroid has BOTH schedule (α) AND weight (0.1)
    - Effective centroid weight: α(t) × 0.1 = [0 → 0.1] (10× weaker than MMD)
...
class VAEConfig:
    ...
    lambda_sparse: float = 0.01  # L1 sparsity on causal matrix G
...
def analyze_dataset_and_suggest_config(
    ...
    if min_ctrl_per_ct < 100:
        batch_size = 64  # Small batches for small datasets
    elif min_ctrl_per_ct < 500:
        batch_size = 128
    elif min_ctrl_per_ct < 2000:
        batch_size = 256
    else:
        batch_size = 512  # Large batches for large datasets
    ...
    if verbose:
        ...
        print(
            f"  - Batch size: {batch_size} (based on {min_ctrl_per_ct} min controls/cell-type)"
        )
...
def train_gcrl_vae(adata, cfg: VAEConfig, *, eigengenes_key: str = "X_comm_eig"):
    ...
    for epoch in epoch_pbar:
        ...
        for batch in batch_pbar:
            ...
            # === 3. L1 Sparsity on causal matrix G ===
            loss_sparse = torch.tensor(0.0, device=device)
            if cfg.lambda_sparse > 0:
                G_upper = torch.triu(model.G, diagonal=1)
                loss_sparse = torch.norm(G_upper, p=1)
                loss = loss + cfg.lambda_sparse * loss_sparse

            # === 4. MMD loss: simulate interventions and compare to real ===
